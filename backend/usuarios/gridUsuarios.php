...
class clsGridUsuarios{
		public function getGrid($ini,$num)
			{
				try {
				$conn = new Conexion();
				$llamarMetodo = $conn->Conectar();
				$sql = "call spGetUsuariosPaged($ini,$num)";
				...
				$result .= "<tr class=\"h6 \">";
				$result .= "<td >Nombre</td><td>id Persona</td><td>Password</td><td>Activo</td><td>Tipo Usuario</td><td>Editar</td><td>Eliminar</td>";
				$result .= "</tr>";
				$result .= "</thead>";
				$result .= "<tbody>";
				while ($row=$stmt->fetch()) {
					$result .= "<tr class=\"h7 \">";
					$result .=  '<td >'.$row[0].'</td><td>'.$row[1].'</td><td>'.$row[2].'</td><td>'.$row[3].'</td><td>'.$row[5].'</td><td>'.  str_replace("REEMPLAZARID",$row[0],$image24)  . '</td><td>'. str_replace("REEMPLAZARID",$row[0],$image25) . '</td>';
					$result .= "</tr>";
				}
				$result .= "</tbody>";
				$result .= "</table>";
			...
			try{
				$conn = new Conexion();
				$llamarMetodo = $conn->Conectar();
				$sql = "call spGetUsuariosTotalCount()";
				$stmt = $llamarMetodo->prepare($sql);
				$stmt->execute();
				$total =0;
				while ($row=$stmt->fetch()) {
